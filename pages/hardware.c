 ...
 * The initial idea was provided by my lab partner, Vinh Vo, inspired by his personal project, the cycloidal gearbox. 
 * [Check his YouTube channel](https://www.youtube.com/channel/UCh_4F4CJVqvAhHmCMTvIb-w). Curious about how we could implement it into a full-motion project, 
 * we decided to use an Inertial Measurement Unit (IMU) to control the cycloidal gearbox. To apply our concepts in Mechanical Engineering with Dynamics, 
 * we decided it would throw a projectile. Our professor, Charlie Refvem, suggested making it wirelessly controlled, specifically using Bluetooth. 
 ...
 * The next step in the process was to design the physical enclosure of the parts. We used SolidWorks to create the CAD models, which included the launcher, 
 * the glove controller, and the cycloidal gearbox. They can be seen below.
 * 
 ...
 * As part of the requirements for this project, our team created a bill of materials, which can be seen 
 * [here](https://raw.githubusercontent.com/jlam94/ME-507-Portfolio/main/images/BOM(UPDATE).xlsx). 
 ...
 * 
 * @subsection custom_pcb Custom PCB Schematics
 * | Page | Description                  |
 * |------|------------------------------|
 ...
 * 
 * @subsection controller_pcb Controller Glove PCB
 ...
 * 
 * @subsection final_product The Final Product
 ...
 * 
 * The next process was to receive the launcher PCB by sending the files to a PCB manufacturer. In our case, we used JLPCB and ordered it with the stencil attached to the frame. 
 * Since it has internal pins and Surface Mount Devices (SMD), the traditional line soldering was not effective, so we used the hot plate method. 
 * We then applied solder paste and spread it, using a hot plate to make the solder paste bind to the PCB. After the parts were bound, 
 * we used heatsinks on top of the motor drivers for heat dissipation in case the parts overheated.
 * 
 ...
 * 
 * The final part that came to us was the PCB for the controller. Unlike the launcher, we used a Blackpill STM32 F411CEU6, so SMD components were not required. 
 * We made a PCB to facilitate the Bluetooth device access and the Inertial Measurement Units (IMU). The rest was hand soldering since they were Arduino headers that go through holes.
 * 
 ...
 *   - **Solution:** The issue is that the rubber band that contacts the ball was press-fitted, and at high speeds, the rubber dilates, blocking the ball from launching. 
 * The solution is to glue the rubber into the CAD wheel. 
